...
export const decodeMemoData: {
  ...
  if (Predicate.isObject(value) && Predicate.hasProperty(value, "name")) {
    return decodeEnvelope(event)(value);
  }

  return decodeEnvelope(event)({ name: event.identifier, data: value });
});
