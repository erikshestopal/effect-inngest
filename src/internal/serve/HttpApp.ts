...
export const toHttpApp = (group: InngestGroup.Any) =>
  Effect.gen(function* () {
    ...
    if (method === "POST") {
      ...
      const result = yield* InternalHandler.handleExecution({
        group,
        fnId: params.fnId,
...
export const toWebHandler = <R, E>(args: {
...
} => HttpEffect.toWebHandlerLayer(toHttpApp(args.group), args.options.layer);
